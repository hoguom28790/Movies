...
export function ComicCard({ title, slug, posterUrl, latestChapter, statusText, originalTitle }: ComicCardProps) {
  return (
    <div className="group relative flex flex-col gap-2 transition-all duration-300">
      <Link href={`/truyen/${slug}`} className="relative aspect-[2/3] w-full overflow-hidden rounded-xl bg-surface transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-lg group-hover:shadow-black/40">
        <Image 
          src={posterUrl} 
          alt={title} 
          fill
