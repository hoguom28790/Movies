            ...
            <div className="flex min-h-full items-center justify-center p-0 sm:p-4 md:p-10 text-center">
              <Transition.Child as={Fragment} enter="ease-out duration-800" enterFrom="opacity-0 translate-y-64" enterTo="opacity-100 translate-y-0" leave="ease-in duration-500" leaveFrom="opacity-100 translate-y-0" leaveTo="opacity-0 translate-y-64">
                <Dialog.Panel className={cn(
                  "relative w-full transform bg-[#0a0a0b] text-left transition-all sm:rounded-[48px] border border-white/5 shadow-2xl flex flex-col overflow-hidden",
                  isXX ? "h-[100vh] sm:h-[90vh] max-w-7xl" : "h-[85vh] max-w-6xl"
                )}>
                  <button onClick={onClose} className={cn(
                    "absolute p-4 rounded-3xl glass-pro text-white hover:text-primary z-[100] border border-white/10 active-depth transition-all",
                  ...
                  <div className="flex-grow overflow-y-auto no-scrollbar scroll-smooth">
                      ...
                      <div className={cn("px-8 sm:px-16 md:px-24", isXX ? "py-12" : "pb-12")}>
                        {isLoading ? (
                           <div className="space-y-12 py-10 opacity-20"><div className="w-40 h-8 bg-white/10 animate-pulse rounded-full" /><div className="w-40 h-8 bg-white/10 animate-pulse rounded-full" /></div>
                        ) : details && (
                          <Tab.Group>
                            <Tab.List className={cn(
                              "flex gap-14 border-b border-white/5 overflow-x-auto no-scrollbar",
                              isXX ? "mb-14 sticky top-0 bg-[#0a0a0b]/90 backdrop-blur-3xl z-[60] py-8 -mx-8 sm:-mx-16 md:-mx-24 px-8 sm:px-16 md:px-24" : "mb-10"
                            )}>
