  ...
  return createPortal(
    <>
      <Transition.Root show={isOpen} as={Fragment}>
        ...
        <AnimatePresence>{toast && <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="fixed bottom-10 right-10 z-[11000] p-6 rounded-2xl glass-pro shadow-2xl border border-white/10"><p className="text-sm font-black text-white uppercase italic">{toast.message}</p></motion.div>}</AnimatePresence>
      </Transition.Root>
    </>,
    document.body
  );
