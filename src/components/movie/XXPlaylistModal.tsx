...
export function XXPlaylistModal({ isOpen, onClose, movieCode, movieTitle, posterUrl }: XXPlaylistModalProps) {
  ...
  return createPortal(
    <AnimatePresence>
      {isOpen && (
        <div 
          className="fixed inset-0 z-[11000] flex items-center justify-center bg-black/85 backdrop-blur-md p-4 overflow-hidden"
          onClick={onClose}
        >
          <motion.div 
            ...
            className="bg-[#141416] p-6 md:p-8 rounded-[40px] border border-white/10 shadow-2xl w-full max-w-md relative flex flex-col gap-6"
